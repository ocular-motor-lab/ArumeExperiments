function [out out2] = PlotPolarHistogram( varargin )
...
% plot_histogram( [ax], [S], data, numberbins, [tit], [xlab], [ylab], [totaltime], [legend], [colors_array] )
%
% plots a polar histogram
%
% options = PlotPolarHistogram( 'get_options' )
% options = PlotPolarHistogram( 'get_defaults' )
%
% PlotPolarHistogram( ax, ...)
...
%   - Use_Lines			= { {'{0}', '1' } };
%   - Number_Of_Bins	= {40};
%   - Fitting           = { {'{None}', 'Von misses 2', 'Von misses 4} };
%
...
%   - numberbins: number of bins of the histogram
...
%
% Examples:
% PlotPolarHistogram( data )
% PlotPolarHistogram( data, numberbins )
% PlotPolarHistogram( S, data, numberbins, tit, xlab, ylab, totaltime )
% PlotPolarHistogram( ax, S, data, numberbins, tit, xlab, ylab, totaltime )
%
if ( nargin == 1 && isstr(varargin{1}))
    ...
    switch (command)
        case 'get_options'
            ...
            Histogram_Options.Fitting           = { {'{None}',  'Von misses 2', 'Von misses 4'} };
...
if ( nargin >=1)
    ...
    p.addOptional('numberbins', [], @(x)(isscalar(x)));
