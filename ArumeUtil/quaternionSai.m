...
%  q  = quaternionSai(x)           x is a matrix size [4,s1,s2,...] or [s1,4,s2,...],
%                               q is size [s1,s2,...], q(i1,i2,...).e = ...
%                               x(1:4,i1,i2,...) or x(i1,1:4,i2,...).'
%  q  = quaternionSai(v)           v is a matrix size [3,s1,s2,...] or [s1,3,s2,...],
%                               q is size [s1,s2,...], q(i1,i2,...).e = ...
%                               [0;v(1:3,i1,i2,...)] or [0;v(i1,1:3,i2,...).']
%  q  = quaternionSai(c)           c is a complex matrix size [s1,s2,...],
%                               q is size [s1,s2,...], q(i1,i2,...).e = ...
%                               [real(c(i1,i2,...));imag(c(i1,i2,...));0;0]
%  q  = quaternionSai(x1,x2)       x1,x2 are matrices size [s1,s2,...] or scalars,
%                               q(i1,i2,...).e = [x1(i1,i2,...);x2(i1,i2,...);0;0]
%  q  = quaternionSai(v1,v2,v3)    v1,v2,v3 matrices size [s1,s2,...] or scalars,
%                               q(i1,i2,...).e = [0;v1(i1,i2,...);v2(i1,i2,...);...
%                               v3(i1,i2,...)]
%  q  = quaternionSai(x1,x2,x3,x4) x1,x2,x3,x4 matrices size [s1,s2,...] or scalars,
%                               q(i1,i2,...).e = [x1(i1,i2,...);x2(i1,i2,...);...
%                               x3(i1,i2,...);x4(i1,i2,...)]
%
% Quaternion array constructor methods:
%  q  = quaternionSai.complexmatrix(Q)
%                               construct quaternions from complex 2x2 matrices
%  q  = quaternionSai.eye(N)       quaternion NxN identity matrix
%  q  = quaternionSai.nan(siz)     q(:).e = [NaN;NaN;NaN;NaN]
%  q  = quaternionSai.ones(siz)    q(:).e = [1;0;0;0]
%  q  = quaternionSai.rand(siz)    uniform random quaternions, NOT Normalized
%                               to 1, 0 <= q.e(1) <= 1, -1 <= q.e(2:4) <= 1
%  q  = quaternionSai.randRot(siz) random quaternions uniform in rotation space
%  q  = quaternionSai.zeros(siz)   q(:).e = [0;0;0;0]
%
% Rotation constructor methods (all lower case):
%  q  = quaternionSai.angleaxis(angle,axis)
...
%  q  = quaternionSai.eulerangles(axes,angles) or
%  q  = quaternionSai.eulerangles(axes,ang1,ang2,ang3)
...
%  q  = quaternionSai.integrateomega(t,w,odeoptions) or
%  q  = quaternionSai.integrateomega(t,omega,axis,odeoptions)
%                               integrate angular velocities over time
%  q  = quaternionSai.modifiedrodrigues(mrp)
%                               quaternions from Modified Rodrigues parameters
%  q  = quaternionSai.rodrigues(rp)
%                               quaternions from Rodrigues parameters
%  q  = quaternionSai.rotateutov(u,v,dimu,dimv)
%                               quaternions Normalized to 1 that rotate 3
%                               element vectors u into the directions of 3
%                               element vectors v
%  q  = quaternionSai.rotationmatrix(R)
...
%                               angular velocity vectors, qd = 0.5*q*quaternionSai(w)
...
%                               help quaternionSai.PropagateEulerEq
...
%  [q,n] = Normalizen(q)         make quaternion norm == 1, unless q == 0,
...
% Array constructors
methods
    function q = quaternionSai( varargin ) % (constructor)
        perm = [];
        sqz  = false;
        switch nargin
            case 0  % nargin == 0
...
%           size(q3) = max( size(q1), size(q2) )
        if ~isa( q1, 'quaternion' )
            q1  = quaternionSai( real(q1), imag(q1), 0, 0 );
        end
        if ~isa( q2, 'quaternion' )
            q2  = quaternionSai( real(q2), imag(q2), 0, 0 );
        end
        s1  = size( q1 );
        s2  = size( q2 );
        nd1 = length( s1 );
    ...
    function qc = conj( q )
        d   = double( q );
        qc  = reshape( quaternionSai( d(1,:), -d(2,:), -d(3,:), -d(4,:) ), ...
                       size( q ));
    end % conj
...
% dim defaults to first dimension of length > 1
        ...
        siz = size( q );
        if siz(1) <= 1
            qd  = quaternionSai.empty;
            return;
        end
        qd  = quaternionSai.zeros( [(siz(1)-1), siz(2:end)] );
        for is = 1 : siz(1)-1
            qd(is,:) = q(is+1,:) - q(is,:);
        end
        ord = ord - 1;
...
% qe.e = exp(q.e(1))*[cos(|v|);v.*sin(|v|)./|v|]
        ...
        qe      = reshape( quaternionSai( de(1,:), de(2,:), de(3,:), de(4,:) ), ...
                           size( q ));
    end % exp
    function ei = imag( q )
        siz = size( q );
...
% quaternion inverse, qi = conj(q)/norm(q)^2, q*qi = qi*q = 1 for q ~= 0
        ...
        di  = Inf( size( d ));
        di(ne0)  = d(ne0) ./ n2(ne0);
        qi  = reshape( quaternionSai( di(1,:), di(2,:), di(3,:), di(4,:) ), ...
                       size( q ));
    end % inverse
...
% logarithm of negative real quaternions is ql.e = [log(|q|);pi;0;0]
        ...
        va(1,nn)= pi;
        dl      = [ lq; va ];
        ql      = reshape( quaternionSai( dl(1,:), dl(2,:), dl(3,:), dl(4,:) ), ...
                           size( q ));
    end % log
    function q3 = minus( q1, q2 )
        ...
        q3  = quaternionSai( d3(1,:), d3(2,:), d3(3,:), d3(4,:) );
        q3  = reshape( q3, siz );
    end % minus
...
% q = quaternions with norm == 1 (unless q == 0), n = former norms
        ...
        ne0 = (n4 ~= 0) & (n4 ~= 1);
        d(ne0)  = d(ne0) ./ n4(ne0);
        q   = reshape( quaternionSai( d(1,:), d(2,:), d(3,:), d(4,:) ), siz );
        if nargout > 1
            n   = shiftdim( n, 1 );
        end
    end % Normalizen
...
% q3 = quaternion product of scalar quaternions q1 and q2
        ...
        eo  = chop( eo );
        q3  = quaternionSai( eo(1), eo(2), eo(3), eo(4) );
    end % product
...
% dim defaults to first dimension of length > 1
    ...
    function qm = uminus( q )
        d   = -double( q );
        qm  = reshape( quaternionSai( d(1,:), d(2,:), d(3,:), d(4,:) ), ...
                       size( q ));
    end % uminus
    function qp = uplus( q )
        qp  = q;
...
% created from 2x2 complex matrices with quaternionSai.complexmatrix(Q)
% (lower case).
% Input:
...
% Output:
%  qd       quaternion derivatives, qd = 0.5 * q * quaternionSai(w)
        if isa( varargin{1}, 'quaternion' )
            qd  = 0.5 .* varargin{1} .* quaternionSai( varargin{2} );
        else
            qd  = 0.5 .* varargin{2} .* quaternionSai( varargin{1} );
        end
    end % Derivative
...
% by spline interpolation. Use q0 = quaternionSai(1,0,0,0) as the initial
...
%  ode45        matlab ode numerical differential equation integrator
        ...
        [~, Y]      = ode45( @Funct, t, y0, options );
            function yd = Funct( ti, yi )
                qi  = quaternionSai( yi(1), yi(2), yi(3), yi(4) );
                qi  = qi.Normalizen;
                wi  = ppval( wpp, ti );
                yd  = double( qi.Derivative( wi ));
            end
        q1          = quaternionSai( Y(:,1), Y(:,2), Y(:,3), Y(:,4) );
        q1          = q1.Normalizen;
%       neg         = real( q1 ) < 0;
...
%         modified 25 July 2012, enforce normalization of q0 and q1
        ...
        t1      = T(:).';
        for it = 1 : nT
            q1(it)   = quaternionSai( Y(it,1), Y(it,2), Y(it,3), Y(it,4) );
            w1(:,it) = Y(it,5:7).';
        end
        q1      = q1.Normalizen;
%       neg     = real( q1 ) < 0;
...
% function q = quaternionSai.angleaxis( angle, axis )
...
%  q        quaternion array
        ...
        q   = quaternionSai( d(1,:), d(2,:), d(3,:), d(4,:) );
        q   = reshape( q, siz );
        if neg == 1
            q   = ipermute( q, perm );
        end
    end % quaternionSai.angleaxis
    function q = complexmatrix( Q )
% function q = quaternionSai.complexmatrix( Q )
...
%           q.e(3) = (c - b)/2, q.e(4) = i*(c + b)/2
        ...
        q   = reshape( quaternionSai( q1, q2, q3, q4 ), siQ(3:end) );
    end % complexmatrix
    function q = eulerangles( varargin )
% function q = quaternionSai.eulerangles( axes, angles )  or
% function q = quaternionSai.eulerangles( axes, ang1, ang2, ang3 )
...
% function q = eye( N )
        ...
        if isempty(N) || (N <= 0)
            ...
            q   = quaternionSai( eye(N), 0, 0, 0 );
        end
    end % quaternionSai.eye
    function q = integrateomega( varargin )
% Integrate angular velocities over time (using ode45) to obtain the
% orientation quaternions at those times, using quaternionSai.Integral and
% initial quaternionSai(1,0,0,0)
%
% Calling syntax 1:
% function q = quaternionSai.integrateomega( t, w, odeoptions )
% Inputs:
%  t(nt)        initial and subsequent (or previous) times t = [t0,t1,...]
%               (monotonic)
%  w(3,nt)      3D angular velocity vectors, radians/(unit time)
...
%
% Calling syntax 2:
% function q = quaternionSai.integrateomega( t, omega, axis, odeoptions )
% Inputs:
%  t(nt)        initial and subsequent (or previous) times t = [t0,t1,...]
%               (monotonic)
%  omega(nt)    angular velocities, radians/(unit time)
...
%  Integral     quaternion integral method
        q   = Integral( quaternionSai(1,0,0,0), varargin{:} );
    end % integrateomega
    function q = modifiedrodrigues( mrp )
% function q = quaternionSai.modifiedrodrigues( mrp )
...
% function q = quaternionSai.nan( siz )
        if isempty( varargin )
            siz = [1 1];
        ...
        if prod( siz ) == 0
            q   = reshape( quaternionSai.empty, siz );
        else
            q   = quaternionSai( nan(siz), nan, nan, nan );
        end
    end % quaternionSai.nan
    function q = NaN( varargin )
% function q = quaternionSai.NaN( siz )
        q   = quaternionSai.nan( varargin{:} );
    end % quaternionSai.NaN
    function q = ones( varargin )
% function q = quaternionSai.ones( siz )
        if isempty( varargin )
            siz = [1 1];
        elseif numel( varargin ) > 1
            siz = [varargin{:}];
        ...
        if prod( siz ) == 0
            ...
            q   = quaternionSai( ones(siz), 0, 0, 0 );
        end
    end % quaternionSai.ones
    function q = rand( varargin )
% function q = quaternionSai.rand( siz )
% Input:
...
%           0 <= q.e(1) <= 1, -1 <= q.e(2:4) <= 1
        ...
        q   = quaternionSai( d(1,:), d(2,:), d(3,:), d(4,:) );
        q   = reshape( q, siz );
    end % quaternionSai.rand
    function q = randRot( varargin )
% function q = quaternionSai.randRot( siz )
...
%           uniform over the 3D surface of a 4 dimensional hypersphere
        ...
        neg = dn(1,:) < 0;
        dn(:,neg) = -dn(:,neg);
        q   = quaternionSai( dn(1,:), dn(2,:), dn(3,:), dn(4,:) );
        q   = reshape( q, siz );
    end % quaternionSai.randRot
    function q = rodrigues( rp )
% function q = quaternionSai.rodrigues( rp )
...
% function q = quaternionSai.rotateutov( u, v, dimu, dimv )
...
%  q        quaternion array
    ...
    end % quaternionSai.rotateutov
    function q = rotationmatrix( R )
% function q = quaternionSai.rotationmatrix( R )
...
%  q        quaternion array
    ...
    end % quaternionSai.rotationmatrix
    function q = zeros( varargin )
% function q = quaternionSai.zeros( siz )
        if isempty( varargin )
            siz = [1 1];
        elseif numel( varargin ) > 1
            siz = [varargin{:}];
        ...
        if prod( siz ) == 0
            ...
            q   = quaternionSai( zeros(siz), 0, 0, 0 );
        end
    end % quaternionSai.zeros
end % methods(Static)
...
q0   = quaternionSai.angleaxis( angles(:).', axis );
qout = q0(1);
for i0 = 2 : numel(q0)
...
s       = sqrt( 0.5 *( 1 - costh ));
eout    = [ c; s * what ];
qout    = quaternionSai( eout(1), eout(2), eout(3), eout(4) );
end % UV2q
% Helper functions
...
% Inputs:
%  v                matrix of vectors
%  dim [OPTIONAL]   dimension to Normalizen, dim >= 1
